...
const SubjectTopologicalSorter_1 = require("./SubjectTopologicalSorter");
...
class SubjectExecutor {
    ...
    async executeInsertOperations() {
        ...
        for (const groupName of groupedInsertSubjectKeys) {
            ...
            else if (this.queryRunner.dataSource.driver.options.type === "oracle") {
                subjects.forEach((subject) => {
                ...
                if (subject.generatedMap) {
                    subject.metadata.columns.forEach((column) => {
                        ...
                        if (value !== undefined && value !== null) {
                            const preparedValue = this.queryRunner.dataSource.driver.prepareHydratedValue(value, column);
    ...
    updateSpecialColumnsInPersistedEntities() {
        ...
        this.allSubjects.forEach((subject) => {
            ...
            // mongo _id remove
            if (InstanceChecker_1.InstanceChecker.isMongoEntityManager(this.queryRunner.manager)) {
                if (subject.metadata.objectIdColumn?.databaseName &&
