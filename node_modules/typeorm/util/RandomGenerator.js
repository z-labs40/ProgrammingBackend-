...
class RandomGenerator {
    ...
    static sha1(str) {
        ...
        for (let blockstart = 0; blockstart < wordArray.length; blockstart += 16) {
            ...
            for (let i = 20; i <= 39; i++) {
                const temp = (_rotLeft(A, 5) + (B ^ C ^ D) + E + W[i] + 0x6ed9eba1) &
                    0x0ffffffff;
                E = D;
                D = C;
                C = _rotLeft(B, 30);
            ...
            for (let i = 60; i <= 79; i++) {
                const temp = (_rotLeft(A, 5) + (B ^ C ^ D) + E + W[i] + 0xca62c1d6) &
                    0x0ffffffff;
                E = D;
                D = C;
                C = _rotLeft(B, 30);
        ...
        const ans = _cvtHex(H0) + _cvtHex(H1) + _cvtHex(H2) + _cvtHex(H3) + _cvtHex(H4);
