...
class PostgresDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = PostgresDriver.supportedIsolationLevels;
        /**
        ...
        this.compareTableIndexTypes = (indexA, indexB) => {
    ...
    async afterConnect() {
        ...
        if (installExtensions) {
            ...
            if (extensionsToInstall) {
                const availableExtensions = await this.getAvailableExtensions(connection);
                await this.enableCustomExtensions(availableExtensions, extensionsToInstall, connection);
            }
        }
        this.isGeneratedColumnsSupported = VersionUtils_1.VersionUtils.isGreaterOrEqual(this.version, "12.0");
        await release();
    }
    async getAvailableExtensions(connection) {
        const availableExtensions = new Set();
        const { logger } = this.dataSource;
        try {
            const result = await this.executeQuery(connection, `SELECT name FROM pg_available_extensions`);
            ...
            logger.log("warn", "Could not retrieve available extensions. Extension installation may fail if extensions are not available.");
        }
    ...
    async enableCustomExtensions(availableExtensions, extensionsToInstall, connection) {
        if (!extensionsToInstall)
            return;
        ...
        for (const extension of extensionsToInstall) {
            if (availableExtensions.has(extension)) {
                try {
                    await this.executeQuery(connection, `CREATE EXTENSION IF NOT EXISTS "${extension}"`);
                }
                catch (_) {
                    logger.log("warn", `The extension "${extension}" cannot be installed automatically. Please install it manually using superuser rights`);
                }
            }
            else {
                logger.log("warn", `The extension "${extension}" is not available on this database. Please install it manually using superuser rights`);
            }
        }
    }
    async checkMetadataForExtensions() {
        const hasUuidColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.generatedColumns.filter((column) => column.generationStrategy === "uuid").length > 0);
        });
        const hasCitextColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.columns.filter((column) => column.type === "citext").length > 0);
        });
        const hasHstoreColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.columns.filter((column) => column.type === "hstore").length > 0);
        });
        const hasCubeColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.columns.filter((column) => column.type === "cube")
                .length > 0);
        });
        const hasGeometryColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.columns.filter((column) => this.spatialTypes.indexOf(column.type) >= 0).length > 0);
        });
        const hasLtreeColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return (metadata.columns.filter((column) => column.type === "ltree")
                .length > 0);
        });
        const hasVectorColumns = this.dataSource.entityMetadatas.some((metadata) => {
            return metadata.columns.some((column) => column.type === "vector" || column.type === "halfvec");
        });
        const hasExclusionConstraints = this.dataSource.entityMetadatas.some((metadata) => {
     ...
     * @param tableColumn
     */
    compareJsonDefaults(columnMetadata, tableColumn) {
        ...
        if (typeof jsonString === "string") {
            try {
                ...
                if (!(err instanceof SyntaxError)) {
                    throw new error_1.TypeORMError(`Failed to compare default values of ${columnMetadata.propertyName} column`);
                }
            }
        }
        else {
            return OrmUtils_1.OrmUtils.deepCompare(columnMetadata.default, jsonString);
        }
        return false;
    }
    /**
            ...
            pool.connect((err, connection, release) => {
                ...
                if (options.logNotifications) {
                    ...
                    connection.on("notification", (msg) => {
                        if (msg) {
                            this.dataSource.logger.log("info", `Received NOTIFY on channel ${msg.channel}: ${msg.payload}.`);
                        }
                    });
                }
                release();
