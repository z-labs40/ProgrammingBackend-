...
const Equal_1 = require("../find-options/operator/Equal");
...
class QueryBuilder {
     ...
     */
    static { this.queryBuilderRegistry = {}; }
    /**
    ...
    constructor(connectionOrQueryBuilder, queryRunner) {
        ...
        if (InstanceChecker_1.InstanceChecker.isDataSource(connectionOrQueryBuilder)) {
            ...
            this.expressionMap = new QueryExpressionMap_1.QueryExpressionMap(this.dataSource);
        }
        else {
    ...
    getQueryAndParameters() {
        ...
        return this.dataSource.driver.escapeQueryWithParameters(query, parameters);
    }
    /**
    ...
    createQueryBuilder(queryRunner) {
        return new this.constructor(this.dataSource, queryRunner ?? this.queryRunner);
    }
    /**
    ...
    createFromAlias(entityTarget, aliasName) {
        ...
        if (this.dataSource.hasMetadata(entityTarget)) {
            ...
            if (typeof entityTarget === "string") {
                const isSubquery = entityTarget.startsWith("(") && entityTarget.endsWith(")");
                return this.expressionMap.createAlias({
                    type: "from",
                    name: aliasName,
    ...
    replacePropertyNamesForTheWholeQuery(statement) {
        ...
        if (replacementKeys.length > 0) {
            statement = statement.replaceAll(new RegExp(
            ...
            `([ =(]|^.{0})` + // any of ' =(' or start of line
                ...
                if (replaceAliasNamePrefixes) {
                    ...
                    if (replacements[matches[2]][p]) {
                        return `${pre}${this.escape(matches[2].slice(0, -1))}.${this.escape(replacements[matches[2]][p])}`;
                    }
                }
                else {
                    match = matches[0];
    ...
    createWhereExpression() {
        ...
        if (this.expressionMap.extraAppendedAndWhereCondition) {
            conditionsArray.push(this.expressionMap.extraAppendedAndWhereCondition);
        }
        let condition = "";
        // time travel
                ...
                case "and":
                    return ((index > 0 ? "AND " : "") +
                        `${this.dataSource.options.isolateWhereStatements
                            ? "("
                            : ""}${expression}${this.dataSource.options.isolateWhereStatements
                            ? ")"
                            : ""}`);
                case "or":
                    return ((index > 0 ? "OR " : "") +
    ...
    createCteExpression() {
        ...
        const databaseRequireRecusiveHint = this.dataSource.driver.cteCapabilities.requiresRecursiveHint;
        const cteStrings = this.expressionMap.commonTableExpressions.map((cte) => {
            ...
            if (typeof cte.queryBuilder !== "string") {
                ...
                if (!this.dataSource.driver.cteCapabilities.writable &&
                    !InstanceChecker_1.InstanceChecker.isSelectQueryBuilder(cte.queryBuilder)) {
                    throw new error_1.TypeORMError(`Only select queries are supported in CTEs in ${this.dataSource.options.type} (CTE: ${cte.alias})`);
                }
            ...
            if (this.dataSource.driver.cteCapabilities.materializedHint &&
    ...
    getWhereInIdsCondition(ids) {
        ...
        if (!metadata.hasMultiplePrimaryKeys) {
            ...
            if (!primaryColumn.transformer &&
                ...
                const values = normalized.map((id) => primaryColumn.getEntityValue(id, false));
                return {
                    [primaryColumn.propertyName]: values.length === 1 ? (0, Equal_1.Equal)(values[0]) : (0, In_1.In)(values),
                };
            }
        }
        ...
        if (InstanceChecker_1.InstanceChecker.isFindOperator(parameterValue)) {
            ...
            if (parameterValue.useParameter) {
                if (parameterValue.objectLiteralParameters) {
                    ...
                    if (parameterValue.type === "jsonContains" &&
                        value !== null &&
                        typeof value === "object") {
