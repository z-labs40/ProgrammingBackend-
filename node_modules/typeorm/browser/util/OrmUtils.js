...
const IsNull_1 = require("../find-options/operator/IsNull");
class OrmUtils {
    // -------------------------------------------------------------------------
    // Public methods
    // -------------------------------------------------------------------------
     ...
     * @param arr2
     */
    static getArraysDiff(arr1, arr2) {
    ...
    static isPrimitiveCriteria(criteria) {
        if (Array.isArray(criteria)) {
            return criteria.every((value) => OrmUtils.isSinglePrimitiveCriteria(value));
        }
    ...
    static compare2Objects(leftChain, rightChain, x, y) {
        ...
        if ((0, Uint8ArrayUtils_1.isUint8Array)(x) && (0, Uint8ArrayUtils_1.isUint8Array)(y)) {
            return (0, Uint8ArrayUtils_1.areUint8ArraysEqual)(x, y);
        }
        ...
        for (p in iterableX) {
            ...
            switch (typeof iterableX[p]) {
                ...
                case "function":
                    leftChain.push(x);
                    rightChain.push(y);
                    if (!OrmUtils.compare2Objects(leftChain, rightChain, iterableX[p], iterableY[p])) {
                        return false;
                    }
                    leftChain.pop();
                    rightChain.pop();
    ...
    static normalizeWhereCriteria(criteria, options, path) {
        ...
        for (const [key, value] of Object.entries(criteria)) {
            ...
            if (value === undefined) {
                ...
                if (behavior === "throw") {
                    throw new error_1.TypeORMError(`Undefined value encountered in property '${propertyPath}' of a where condition. ` +
                ...
                if (behavior === "throw") {
                    throw new error_1.TypeORMError(`Null value encountered in property '${propertyPath}' of a where condition. ` +
                ...
                !InstanceChecker_1.InstanceChecker.isFindOperator(value)) {
                const nested = OrmUtils.normalizeWhereCriteria(value, options, propertyPath);
