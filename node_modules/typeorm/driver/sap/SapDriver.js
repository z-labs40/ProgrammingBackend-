...
class SapDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = SapDriver.supportedIsolationLevels;
        /**
    ...
    async connect() {
        ...
        const poolOptions = {
            ...
            maxWaitTimeoutIfPoolExhausted: this.options.pool?.maxWaitTimeoutIfPoolExhausted ?? 0,
        };
        ...
        this.poolErrorHandler =
            this.options.pool?.poolErrorHandler ??
                ((error) => {
                    this.dataSource.logger.log("warn", `SAP HANA pool raised an error: ${error}`);
                });
        // create the pool
        try {
    ...
    async disconnect() {
        ...
        if (!pool) {
            throw new ConnectionIsNotSetError_1.ConnectionIsNotSetError("sap");
        }
